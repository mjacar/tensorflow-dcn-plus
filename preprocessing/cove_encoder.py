# Slightly modified version of https://github.com/salesforce/cove/blob/master/cove/encoder.py
import os
...
model_cache = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), '.torch')
...
class MTLSTM(nn.Module):
  ...
  def __init__(self, n_vocab=None, vectors=None, residual_embeddings=False):
    ...
    self.rnn.load_state_dict(model_zoo.load_url(
        model_urls['wmt-lstm'], model_dir=model_cache))
